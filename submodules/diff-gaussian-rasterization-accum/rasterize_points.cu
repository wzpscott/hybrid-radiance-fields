	...
	const bool debug)
{
  ...
  torch::Tensor weight = torch::full({P}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_alpha, weight);
}
	...
	const bool debug)
{
  ...
  {  
	  ...
	  dL_dout_invdepthptr,
	  dL_dout_T.contiguous().data<float>(),
