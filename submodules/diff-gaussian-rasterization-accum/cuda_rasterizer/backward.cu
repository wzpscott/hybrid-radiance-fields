...
renderCUDA(
	...
	const float* __restrict__ dL_dTs,
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Update gradients w.r.t. the background blending
			...
			atomicAdd(&dL_dmean2D[global_id].z, (fabs(dL_dG * dG_ddelx * ddelx_dx) + fabs(dL_dG * dG_ddely * ddely_dy)) / 4);
